<div>
    ...
    <flux:card class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <flux:heading size="xl" class="mb-1">{{ $quiz->title }}</flux:heading>
                <flux:subheading>Manage Questions ({{ $quiz->questions->count() }} total)</flux:subheading>
            </div>
            <flux:button
                href="{{ auth()->user()->role === 'admin' ? route('admin.quizzes.view', $quiz) : route('organization.quizzes.view', $quiz) }}"
                variant="ghost">
    ...
    <flux:card>
        ...
        @else
            <div class="text-center py-8 text-zinc-500">
                ...
                <flux:text>No questions added yet. Add your first question above.</flux:text>
            </div>
        @endif
    </flux:card>
</div>
